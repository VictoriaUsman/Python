...
def save():
    with open('Keeper.txt', 'a') as file:
        ...
        file.write(f"{web} | Username: {user} |Password: {password}\n")
        messagebox.showinfo(title = "File Saved", message = f"File saved successfully")
        entry.delete(0, END)
        user_entry.delete(0, END)
        password_entry.delete(0, END)
        user_entry.insert(0, "iantrisdc@gmail.com")
        messagebox.showinfo(title = "Password Copied", message = f"Password copied successfully")
